    ...
    public class Labyrinth : ILabyrinth {
        ...
        public void SpawnHero(IHero hero) { // spawn hero only in ground or coin cells 
            var cells = Cells.Where(cell =>
                cell.Val == CellType.Ground
                || cell.Val == CellType.Coin);
            ...
            HeroSpawnFlag = true;
            if (cellsList[randIndex].Val == CellType.Coin) RemoveCoin(hero.X, hero.Y);
            GetCoinsCount();
        ...
        public void RemoveCoin(int x, int y) {
            this[x, y].Val = CellType.Ground; // coin -> pass
        }
        ...
        public void CheckCoin(int x, int y) {
            ...
            if (this[x, y].Val != CellType.Coin) return;
            hero.AddCoin();
            RemoveCoin(x,y);
        }
